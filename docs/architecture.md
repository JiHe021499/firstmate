...
A `https://<host>/<path>/-/merge_requests/<n>` URL, or the `https://<host>/<path>/merge_requests/<n>` route an instance older than GitLab 12.0 serves (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)), invokes `glab mr merge <n> -R https://<host>/<path>`, so the instance comes from the URL, and adds no merge-method flag because the project's own merge method applies.
...
An instance that reports no pipeline is told it has no CI, and one that reports no `detailed_merge_status` is told its mergeability could not be read, because the fields such an instance omits arrive as their type defaults; both absences refuse, and neither is ever a pass.
