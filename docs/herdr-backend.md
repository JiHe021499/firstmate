...
A structurally gone pane or a pane read from a session positively reported as having no running server becomes `missing`, a restored agent-less shell becomes `dead`, a registered agent whose pane still runs an agent process becomes `alive`, and every other unexpected read becomes `unreadable`.
A registration whose pane holds only its own shell is stale, so it classifies `dead` rather than `alive`.
